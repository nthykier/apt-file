...
action is very close to the B<dpkg -L> command except the package does
...
The B<apt-file> command will only search for filenames, not directory
names.  This is due to the format of the Contents files it searches.
...
Only search indexes of the given type(s).  The type(s) must match one
...
Then, apt-file will recognise "deb", "dsc" and "udeb" as index types.
...
Chooses which Contents files to download.
