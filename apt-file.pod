...
List the Contents indices registrered in APT and their global default
status in a human readable format.  These indices can be searched via
the B<-I> option (when enabled and after fetching the indices).

Note that individual sources.list entries can override the global
default.

For a machine-readable format, please use L<apt-config(1)> (search for
B<Acquire::IndexTargets>).
